...
class GauzeRetrieveAutoGraspCurriculumLearning(PsmEnv):
    """
    ...
    def _env_setup(self):
        super(GauzeRetrieveAutoGraspCurriculumLearning, self)._env_setup()
        ...
        if alg == 'ddpgcl':
            file_path = './logs/ddpgcl/GauzeRetrieveAutoGraspCurriculumLearning-1e5_0/progress.csv'
            try:
            ...
            file_path = './logs/hercl/GauzeRetrieveAutoGraspCurriculumLearning-1e5_1/progress.csv'
            try:
                data = pd.read_csv(file_path)
                if data.empty:
                    epoch = 0
    ...
    def _set_action(self, action: np.ndarray):
        ...
        super(GauzeRetrieveAutoGraspCurriculumLearning, self)._set_action(action)
    ...
    def _sample_goal_callback(self):
        ...
        self._waypoints[0] = np.array([pos_obj[0], pos_obj[1],
                                       pos_obj[2] + (-0.0007 + 0.0102 + 0.005) * self.SCALING, 0., 0])  # approach
        self._waypoints[1] = np.array([pos_obj[0], pos_obj[1],
                                       pos_obj[2] + (-0.0007 + 0.0102) * self.SCALING, 0., 0])  # approach
        self._waypoints[2] = np.array([pos_obj[0], pos_obj[1],
                                       pos_obj[2] + (-0.0007 + 0.0102) * self.SCALING, 0., 0])  # grasp
        self._waypoints[3] = np.array([pos_obj[0], pos_obj[1],
                                       pos_obj[2] + (-0.0007 + 0.0102 + 0.005) * self.SCALING, 0., 0])  # grasp
        self._waypoints[4] = np.array([self.goal[0], self.goal[1],
                                       self.goal[2] + 0.0102 * self.SCALING, 0., 0])  # lift up
...
if __name__ == "__main__":
    env = GauzeRetrieveAutoGraspCurriculumLearning(render_mode='human')  # create one process and corresponding env
