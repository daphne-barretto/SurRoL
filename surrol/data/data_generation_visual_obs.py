...
def main():
    ...
    # Convert lists to arrays for consistent episode-first shape
    np_actions = np.array(actions)
    np_observations = np.array(observations, dtype=object)
    np_infos = np.array(infos, dtype=object)
    np_rewards = np.array(rewards)
    np_images = np.array(all_images, dtype=object)  # shape: (num_episodes, steps, H, W, C)

    np.savez_compressed(os.path.join(folder, file_name),
                        acs=np_actions, obs=np_images, infos=np_infos,
                        rewards=np_rewards, traditional_obs=np_observations)
    ...
    if args.video:
        ...
        for episode_images in all_images:
...
def goToGoal(env, last_obs):
    ...
    # adds the initial observation to the list of images
    # if args.video:
    ...
    if success:
        ...
        if args.video:
            all_images.append(episode_images)
            if episode_masks:
                all_masks.append(episode_masks)
