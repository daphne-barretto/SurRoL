...
def main():
    ...
    for alg_dir in os.listdir(args.logs_dir):
        ...
        for run_dir in os.listdir(os.path.join(args.logs_dir, alg_dir)):
            if run_dir.startswith(args.task):
                ...
                if 'train/success_rate' not in results:
                    results['train/success_rate'] = calculate_success_rate(results, args.task)
                success_rate = np.array(results['train/success_rate'])
    ...
    if args.plot_separate:
        plt.savefig("./plotting/training_pngs/" + args.task + "_separate.png", dpi=300)
    else:
        plt.savefig("./plotting/training_pngs/" + args.task + ".png", dpi=300)
    plt.close()
